...
class ResBasicBlock(nn.Module):
    def __init__(self, n_channels, n_inner_channels, kernel_size=3, num_groups=8):
        ...
        self.norm1 = nn.BatchNorm2d(n_inner_channels, track_running_stats=False) #nn.GroupNorm(num_groups, n_inner_channels)
        self.norm2 = nn.BatchNorm2d(n_channels, track_running_stats=False) #nn.GroupNorm(num_groups, n_channels)
        self.norm3 = nn.BatchNorm2d(n_channels, track_running_stats=False) #nn.GroupNorm(num_groups, n_channels)
...
class ResDEQ(nn.Module):
    def __init__(self, in_channels, deq_channels, inner_channels, num_classes, tol=1e-2, max_iter=50) -> None:
        ...
        self.bn1 = nn.BatchNorm2d(deq_channels, track_running_stats=False)
        self.deq = DEQFixedPoint(ResBasicBlock(deq_channels, inner_channels), anderson, tol=tol, max_iter=max_iter)
        self.bn2 = nn.BatchNorm2d(deq_channels, track_running_stats=False)
