...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        t0, tN = self.integration_times
        middle = self.tt(x.reshape(1,-1))
        temp = odeint(self.odefunc, x, torch.tensor([t0, middle], requires_grad=True), rtol=args.tol, atol=args.tol)[-1]
        out = odeint(self.odefunc, temp, torch.tensor([middle, tN], requires_grad=True), rtol=args.tol, atol=args.tol)
...
if __name__ == '__main__':
    ...
    model = ODEBlock(ODEfunc(dim, nhidden), t0, tN, ids)
    # model = mySequential(*feature_layers).to(device)
    ...
    for itr in range(1, args.niters + 1):
        ...
        print(model.odefunc.fc1.weight.grad)
        print(model.tt.fc1.weight.grad)
